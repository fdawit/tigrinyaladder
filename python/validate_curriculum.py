...
def collect_ethiopic_tokens(value):
    tokens = set()
    pattern = re.compile(r"[\u1200-\u135A]+")
    def walk(item):
        if isinstance(item, str):
            tokens.update(pattern.findall(item))
        elif isinstance(item, list):
            for child in item:
                walk(child)
        elif isinstance(item, dict):
            for key, child in item.items():
                if key == "plainPhonetics":
                    continue
                walk(child)
    walk(value)
    return tokens
...
def main() -> None:
    ...
    corpus_tokens = collect_ethiopic_tokens({
        "lessons": data.get("lessons", []),
        "checkpointBanks": data.get("checkpointBanks", {}),
        "glossary": data.get("glossary", []),
        "glossaryMeta": data.get("glossaryMeta", {}),
        "readingLadder": data.get("readingLadder", []),
    })
    plain_words = set(data.get("plainPhonetics", {}).get("words", {}))
    roman_hints = set(data.get("romanizationHints", {}))
    missing_plain = sorted(corpus_tokens - plain_words)
    missing_roman = sorted(corpus_tokens - roman_hints)
    if missing_plain:
        errors.append(f"Missing plain-English cue entries for {len(missing_plain)} corpus tokens: {missing_plain[:20]}")
    if missing_roman:
        errors.append(f"Missing romanization/reading-support entries for {len(missing_roman)} corpus tokens: {missing_roman[:20]}")
