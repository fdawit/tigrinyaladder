...
(() => {
  ...
  const phoneticToggle = document.querySelector('#phoneticToggle');
  ...
  const phoneticWordMap = new Map(Object.entries(data.plainPhonetics?.words || {}));
  const phoneticCharMap = new Map(Object.entries(data.plainPhonetics?.characters || {}));
  const ETHIOPIC_WORD_RE = /[\u1200-\u135A]+/g;
  ...
  function normalizeSettings(saved = {}) {
    return {
      showPlainPhonetics: saved.showPlainPhonetics !== false,
    };
  }

  function plainPhoneticsVisible() {
    return state.progress.settings?.showPlainPhonetics !== false;
  }

  function applyPhoneticsVisibility() {
    const visible = plainPhoneticsVisible();
    document.body.classList.toggle('hide-phonetics', !visible);
    if (phoneticToggle) {
      phoneticToggle.textContent = visible ? 'Hide plain cues' : 'Show plain cues';
      phoneticToggle.setAttribute('aria-pressed', String(visible));
      phoneticToggle.setAttribute('title', visible ? 'Hide approximate plain-English phonetic cues' : 'Show approximate plain-English phonetic cues');
    }
  }
  ...
  function tigrinyaTokens(value) {
    return String(value ?? '').match(ETHIOPIC_WORD_RE) || [];
  }

  function romanFor(value) {
    const key = String(value ?? '').trim();
    if (romanMap.has(key)) return romanMap.get(key);
    if (!hasEthiopic(key)) return '';
    return plainPhoneticsFor(key);
  }

  function plainifyRoman(value = '') {
    return String(value)
  ...
  function plainPhoneticsFor(value, explicitRoman = '') {
    const text = String(value ?? '').trim();
    if (!hasEthiopic(text)) return '';
    if (explicitRoman) return plainifyRoman(explicitRoman);
    if (phoneticWordMap.has(text)) return phoneticWordMap.get(text);
    if (romanMap.has(text)) return plainifyRoman(romanMap.get(text));
    const tokens = tigrinyaTokens(text);
    if (!tokens.length) return '';
    return tokens.map(token => {
      if (phoneticWordMap.has(token)) return phoneticWordMap.get(token);
      if (romanMap.has(token)) return plainifyRoman(romanMap.get(token));
      return Array.from(token).map(ch => phoneticCharMap.get(ch) || ch).join('-');
    }).join(' ');
  }
  ...
  function romanHintHTML(value, supportStage = 'guided', explicitRoman = '') {
    ...
    const isGenerated = !explicitRoman && !romanMap.has(String(value ?? '').trim());
    const generatedNote = isGenerated ? '<span class="guide-note">generated from the course phonetic map</span>' : '';
    if (supportStage === 'beginner') {
      return `<span class="roman-hint" aria-label="romanisation guide" title="Rough no-audio guide, not a full pronunciation system">${romanHTML}</span>${generatedNote}`;
    }
    const summary = supportStage === 'fidel' ? 'reveal reading support' : 'pronunciation guide';
    return `<details class="roman-details"><summary>${summary}</summary><span class="roman-hint">${romanHTML}</span>${generatedNote}</details>`;
  }

  function phoneticHintHTML(value, explicitRoman = '') {
    if (!plainPhoneticsVisible()) return '';
    const plain = plainPhoneticsFor(value, explicitRoman);
    if (!plain) return '';
    return `<span class="phonetic-hint" aria-label="plain English phonetic guide"><span class="phonetic-label">plain:</span> ${escapeHTML(plain)}</span>`;
  }

  function formatChoice(value, supportStage = 'guided') {
    const safe = escapeHTML(value);
    if (!hasEthiopic(value)) return safe;
    return `<span class="choice-content"><span class="tg">${safe}</span>${romanHintHTML(value, supportStage)}${phoneticHintHTML(value)}</span>`;
  }

  function stepPhoneticsPanelHTML(step) {
    if (!step || !plainPhoneticsVisible()) return '';
    const found = [];
    const seen = new Set();
    function addFrom(value) {
      tigrinyaTokens(value).forEach(token => {
        if (seen.has(token)) return;
        seen.add(token);
        found.push({ token, plain: plainPhoneticsFor(token) });
      });
    }
    function walk(value) {
      if (typeof value === 'string') addFrom(value);
      else if (Array.isArray(value)) value.forEach(walk);
      else if (value && typeof value === 'object') Object.values(value).forEach(walk);
    }
    walk({ title: step.title, prompt: step.prompt, body: step.body, examples: step.examples, choices: step.choices, bank: step.bank, answer: step.answer, pairs: step.pairs, rows: step.rows });
    if (!found.length) return '';
    const visible = found.slice(0, 28);
    const more = found.length > visible.length ? `<p class="small">+${found.length - visible.length} more in this card. Longer strings are also broken down character-by-character if no word guide exists.</p>` : '';
    return `<details class="plain-phonetics-panel"><summary>Plain English phonetics in this card</summary><div class="phonetic-chip-grid">${visible.map(item => `<span class="phonetic-chip"><strong class="tigrinya">${escapeHTML(item.token)}</strong><em>${escapeHTML(item.plain || '')}</em></span>`).join('')}</div>${more}<p class="small">These cues are shown because the site has no audio. They help you read; they are not proof of pronunciation accuracy.</p></details>`;
  }
  ...
  function readingLadderHTML(compact = false) {
    ...
    return `<section class="panel reading-ladder-panel"><p class="eyebrow">Visible reading ladder</p><h2>How reading grows in this no-audio course.</h2><div class="reading-ladder ${compact ? 'compact' : ''}">${ladder.map(item => {
      const label = item.name || item.title || `Level ${item.level}`;
      const detail = item.canDo || item.description || '';
      const example = item.example || '';
      return `
      <article class="reading-rung">
        <span class="rung-number">${escapeHTML(item.level)}</span>
        <div><h3>${escapeHTML(label)}</h3><p>${escapeHTML(detail)}</p>${example ? `<p class="small tigrinya">${escapeHTML(example)}</p>` : ''}</div>
      </article>`;
    }).join('')}</div></section>`;
  }

  function conversationLadderHTML() {
    const ladder = data.conversationLadder || [];
    if (!ladder.length) return '';
    return `<section class="panel reading-ladder-panel"><p class="eyebrow">Everyday conversation ladder</p><h2>How Explore More becomes more conversational.</h2><div class="reading-ladder">${ladder.map(item => `
      <article class="reading-rung">
        <span class="rung-number">${escapeHTML(item.level)}</span>
        <div><h3>${escapeHTML(item.name)}</h3><p>${escapeHTML(item.canDo)}</p></div>
      ...
      <section class="lesson-layout">
        ...
        <article class="lesson-shell">
          <header class="lesson-header">
            ...
            <div class="support-banner"><strong>${escapeHTML(supportStageLabel(supportStage))}</strong><span>${escapeHTML(supportStageDescription(supportStage))}</span><span><strong>Plain English cues:</strong> ${plainPhoneticsVisible() ? 'shown for every course word/token as a rough no-audio bridge.' : 'hidden. Pronunciation-guide details can still be opened when lesson-stage support is available.'}</span></div>
  ...
  function cardHTML(title, body, step = null) {
    const status = '';
    const phonetics = step ? stepPhoneticsPanelHTML(step) : '';
    return `<div class="step-card"><h3>${escapeHTML(title)}</h3>${status}${body}${phonetics}</div>`;
  }
  ...
  function examplesHTML(examples = [], supportStage = 'guided') {
    ...
    return `<div class="example-grid">${examples.map(ex => `
      <div class="example-card">
        <div class="example-script"><p class="tigrinya">${escapeHTML(ex.tg)}</p>${romanHintHTML(ex.tg, supportStage, ex.roman || '')}${phoneticHintHTML(ex.tg, ex.roman || '')}</div>
  ...
  function fidelSystemHTML() {
    const rows = data.plainPhonetics?.familyRows || data.fidel.families.map(family => ({ base: family.base, name: family.name, plainFamily: family.name, note: '', orders: family.orders.map((char, i) => ({ char, order: i + 1, plain: plainPhoneticsFor(char) })) }));
    const orderRows = data.plainPhonetics?.orderPlainNames || data.fidel.orderNames.map((name, index) => ({ order: String(index + 1), plain: name, use: '' }));
    return `<section class="panel fidel-system-panel"><p class="eyebrow">Complete Fidel System</p><h2>Family × seven orders</h2><p>The learner's job is to master the grid, not memorize isolated symbols. Move in this order: <strong>family shape → order position → plain cue → real word</strong>.</p><div class="fidel-order-strip">${orderRows.map((row, index) => `<article><strong>${index + 1}</strong><span>${escapeHTML(row.plain)}</span><em>${escapeHTML(row.use || '')}</em></article>`).join('')}</div><div class="table-wrap fidel-system-wrap"><table class="fidel-system-table"><thead><tr><th>Family</th>${data.fidel.orderNames.map((name, i) => `<th>${i + 1}<br><span>${escapeHTML(name)}</span></th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr><th><span class="tigrinya">${escapeHTML(row.base)}</span><small>${escapeHTML(row.plainFamily)} · ${escapeHTML(row.note || '')}</small></th>${row.orders.map(cell => `<td><strong class="tigrinya">${escapeHTML(cell.char)}</strong>${plainPhoneticsVisible() ? `<span class="phonetic-hint"><span class="phonetic-label">plain:</span> ${escapeHTML(cell.plain)}</span>` : ''}</td>`).join('')}</tr>`).join('')}</tbody></table></div><div class="method-grid"><article class="method-card"><h3>1. Family</h3><p>Ask which base shape the character belongs to.</p></article><article class="method-card"><h3>2. Order</h3><p>Ask which of the seven vowel positions it occupies.</p></article><article class="method-card"><h3>3. Word</h3><p>Use the character inside a familiar word, phrase, or sentence.</p></article></div></section>`;
  }

  function renderFidel() {
    const family = data.fidel.families.find(item => item.base === state.currentFamily) || data.fidel.families[0];
    app.innerHTML = `
      <section class="section-title"><div><p class="eyebrow">Fidel Lab</p><h2>Master the whole system, then drill one family.</h2><p class="lead">Study the Geʽez/Ethiopic script as a 25-family × 7-order grid. Use the plain-cue toggle when you want support, then hide cues for a fidel-first challenge.</p></div><button class="primary-button" data-practice-mode="fidel-fast" data-view-jump="practice">Fidel Fast Match</button></section>
      ${fidelSystemHTML()}
      <section class="panel">
        <div class="fidel-toolbar">${data.fidel.families.map(item => `<button class="chip-button ${item.base === family.base ? 'active' : ''}" data-family="${escapeHTML(item.base)}"><span class="tigrinya">${escapeHTML(item.base)}</span> ${escapeHTML(item.name)} <span class="small">${escapeHTML(item.plainFamilyName || '')}</span></button>`).join('')}</div>
        <article class="fidel-card">
          ...
          <div class="fidel-grid">${family.orders.map((char, i) => `<div class="fidel-cell"><strong class="tigrinya">${escapeHTML(char)}</strong><span>${escapeHTML(data.fidel.orderNames[i])}</span>${phoneticHintHTML(char)}</div>`).join('')}</div>
  ...
  function renderGuide() {
    ...
    app.innerHTML = `
      ...
      <section class="panel"><h2>Language coverage map</h2><p>This course is organized as a simple Core Path plus a deeper Explore More library. The Core Path stays beginner-safe: romanization scaffolding, fidel basics, greetings, pronouns, identity sentences, and mini-conversations. Explore More has been revised around everyday Eritrean/Asmara-style conversation: family identity, names, age, school, work, visits, hospitality, phone/texting, plans, public places, errands, care/comfort, relationship language, reading-to-reply tasks, and only then the grammar map that explains those forms.</p><p><strong>Dialect policy:</strong> this version uses Eritrean/Asmara Tigrinya as the default classroom variety. It avoids presenting Tigray/Ethiopian regional alternatives, liturgical forms, or formal written variants as main-path beginner answers.</p><p><strong>Romanization and plain phonetics policy:</strong> early lessons show rough Eritrean/Asmara-oriented romanization because there is no audio. Middle lessons hide romanization behind pronunciation-guide hints. Plain-English phonetics can be shown or hidden with the header toggle. They are a reading bridge for every Tigrinya word and fidel character, while later review can emphasize fidel-first recognition.</p><p><strong>Important limit:</strong> no-audio learning cannot fully teach pronunciation, listening, intonation, or live fluency. Use this app as a thinking scaffold, then pair it with Eritrean/Asmara native audio and fluent-speaker correction.</p></section>
      ...
      ${data.resourceDigest ? `<section class="panel"><p class="eyebrow">Uploaded resource digest</p><h2>How the uploaded references shaped the source-expanded curriculum</h2><div class="method-grid">${data.resourceDigest.map(item => `<article class="method-card"><h3>${escapeHTML(item.source)}</h3><p>${escapeHTML(item.takeaway)}</p></article>`).join('')}</div><p class="small">Scanned pages were used through visible page images and available parsed text; examples are still marked for fluent Eritrean/Asmara review.</p></section>` : ''}
      ${readingLadderHTML(false)}
      ${conversationLadderHTML()}
      ${fidelSystemHTML()}
  ...
  phoneticToggle?.addEventListener('click', () => {
    ...
    state.progress.settings.showPlainPhonetics = !plainPhoneticsVisible();
    saveProgress();
    applyPhoneticsVisibility();
    render();
  });
